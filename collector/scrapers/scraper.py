...
class F1DataCollector:
    ...
    def get_schedule(self):
        ...
        for i, event in enumerate(schedule):
            ...
            if not event.get("dates") and event.get("sessions"):
                try:
                    s_times = [datetime.datetime.fromisoformat(s['time'].replace('Z', '+00:00')) for s in event["sessions"]]
                    if s_times:
                        s_times.sort()
                        start = s_times[0]
                        end = s_times[-1]
                        ...
                        if start.month == end.month:
                            event["dates"] = f"{start.day:02d} - {end.day:02d} {months[start.month-1]}"
                        else:
                            event["dates"] = f"{start.day:02d} {months[start.month-1]} - {end.day:02d} {months[end.month-1]}"
                        print(f"  -> Generated dates for {event['slug']}: {event['dates']}")
                except Exception as e:
